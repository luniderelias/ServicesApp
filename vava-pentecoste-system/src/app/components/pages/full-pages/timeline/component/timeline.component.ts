...
export class TimelineComponent implements AfterViewInit {
  ...
  private static getElementWidth(element: Element): number {
    ...
    return TimelineComponent.pxToNumber(computedStyle.width);
  }
  ...
  private static minLapse(elements: TimelineElement[]): number {
    ...
    for (let i = 1; i < elements.length; i++) {
      let distance = TimelineComponent.dayDiff(elements[i - 1].date, elements[i].date);
  ...
  updateSlide(timelineTotWidth: number, forward: boolean) {
    let translateValue = TimelineComponent.getTranslateValue(this.eventsWrapper.nativeElement);
  ...
  updateTimelinePosition(element: Element) {
    ...
    let eventLeft = TimelineComponent.pxToNumber(eventStyle.getPropertyValue('left'));
    let translateValue = TimelineComponent.getTranslateValue(this.eventsWrapper.nativeElement);
  ...
  private updateFilling(element: any) {
    ...
    let eventLeftNum = TimelineComponent.pxToNumber(eventLeft) + TimelineComponent.pxToNumber(eventWidth) / 2;
    let scaleValue = eventLeftNum / this.eventsWrapperWidth;
    TimelineComponent.setTransformValue(this.fillingLine.nativeElement, 'scaleX', scaleValue);
  }
